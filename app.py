...
st.sidebar.header("Manage Decks")
available_decks = storage.get_all_decks()
selected_deck = st.sidebar.selectbox("Select a Deck", available_decks)

if not selected_deck:
    new_deck_name = st.sidebar.text_input("Create a new deck:")
    if st.sidebar.button("Create Deck"):
        if new_deck_name:
            storage.save_deck(new_deck_name, {})
            st.rerun()

st.session_state.user_progress = storage.load_deck(selected_deck)

try:
    deck = pd.read_csv("data/sample_deck.csv")
except FileNotFoundError:
    st.error("`sample_deck.csv` not found! Please ensure it's in the `data/` directory.")
    st.stop()
...
if not st.session_state.user_progress:
    ...
    storage.save_deck(selected_deck, st.session_state.user_progress)
    ...
    if ai_note:
        with st.spinner("Generating..."):
            ...
            for i in range(num_questions):
                try:
                    prompt = f"Generate a single flashcard question from the following note: {ai_note}. Question must be different from previous questions generated."
...
if page == "Dashboard":
    ...
    if review_dates:
        ...
        ax.bar([d.strftime('%Y-%m-%d') for d in sorted_dates], [date_counts[d] for d in sorted_dates])
    ...
    uploaded_file = st.file_uploader("Upload a CSV file with 'Question' and 'Answer' columns", type="csv")
    ...
    current_deck_data_json = json.dumps(st.session_state.user_progress, indent=4)
    st.download_button(
        label=f"Download '{selected_deck}' Deck Data",
        data=current_deck_data_json,
        file_name=f"{selected_deck}.json",
